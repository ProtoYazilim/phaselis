...
```jsx  
<Checkbox iconName="X" text="Checkbox" />
```
...
<Checkbox
  IconSlot={() => (
    <Image source={{ uri: "https://example.com/remote-image.jpg" }} />
  )}
/>
```
### name
...
```jsx
<Checkbox name="1" />
<Checkbox name="2" partofform={false} />
```
...
```jsx
<Form ref={refForm}>
  <Checkbox name="1" value={true} resetValue={false} />
